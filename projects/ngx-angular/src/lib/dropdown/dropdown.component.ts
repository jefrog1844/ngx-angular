...
export class DropdownComponent implements OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    if (this.alignment) {
      this.renderer.addClass(
        this.menuEl,
        `mui-dropdown__menu--${this.alignment}`
      );
    }
  ...
  handleDocumentClick(targetEl: HTMLElement): void {
    if (!this.wrapperEl.contains(targetEl)) {
      this.open.next(false);
    }
  }
  ...
  openDropdown(): void {
    ...
    switch (this.placement) {
      case 'up':
        pos.bottom =
          buttonRect.height + buttonRect.top - wrapperRect.top + 'px';
        break;
      case 'right':
      ...
      case 'left':
        ...
        break;
      default:
        pos.top = buttonRect.top - wrapperRect.top + buttonRect.height + 'px';
    }
